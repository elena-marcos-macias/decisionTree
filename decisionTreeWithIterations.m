...
CoordBestAUC = ones(10, 2, 3); % [points to plot, axes, model] -> axes = [X, Y]
    CurrentBestauc = zeros(1,3); % [auc value, model] 
CoordWorstAUC = ones(10, 2, 3); % [points to plot, axes, model] -> axes = [X, Y]
    CurrentWorstauc = ones (1,3); % [auc value, model] 
...
for run = 1:nRuns
    ...
    if CurrentBestauc(1) < auc1
        ...
        CoordBestAUC ((1:length(fp1)),:,1) = [fp1, tp1];
    end
    if CurrentWorstauc(1) > auc1
        CurrentWorstauc(1) = auc1;
        CoordWorstAUC ((1:length(fp1)),:,1) = [fp1, tp1];
    end
    ...
    if CurrentBestauc(2) < auc2
        ...
        CoordBestAUC ((1:length(fp2)),:,2) = [fp2, tp2];
    end
    if CurrentWorstauc(2) > auc2
        CurrentWorstauc(2) = auc2;
        CoordWorstAUC ((1:length(fp2)),:,2) = [fp2, tp2];
    end
    ...
    if CurrentBestauc(3) < auc3
        ...
        CoordBestAUC ((1:length(fp3)),:,3) = [fp3, tp3];
    end
    if CurrentWorstauc(3) > auc3
        CurrentWorstauc(3) = auc3;
        CoordWorstAUC ((1:length(fp3)),:,3) = [fp3, tp3];
    end
...
%% ROC CURVES
Bfp1 = CoordBestAUC(:,1,1);
Btp1 = CoordBestAUC(:,2,1);
PlotBauc1 = CurrentBestauc(:,1);

Wfp1 = CoordWorstAUC(:,1,1);
Wtp1 = CoordWorstAUC(:,2,1);
PlotWauc1 = CurrentWorstauc(:,1);

Bfp2 = CoordBestAUC(:,1,2);
Btp2 = CoordBestAUC(:,2,2);
PlotBauc2 = CurrentBestauc(:,2);

Wfp2 = CoordWorstAUC(:,1,2);
Wtp2 = CoordWorstAUC(:,2,2);
PlotWauc2 = CurrentWorstauc(:,2);

Bfp3 = CoordBestAUC(:,1,3);
Btp3 = CoordBestAUC(:,2,3);
PlotBauc3 = CurrentBestauc(:,3);

Wfp3 = CoordWorstAUC(:,1,3);
Wtp3 = CoordWorstAUC(:,2,3);
PlotWauc3 = CurrentWorstauc(:,3);
